...
Object::Object(float t, int id, int cls, int x, int y, int w, int h) : t(t),
                                                                       id(id),
                                                                       cls(cls),
                                                                       x(x), y(y), w(w), h(h)
{
...
static void calc_velocity()
{
    for (auto &obj : data)
    {
        ...
        if (obj.lt != -1){
            float d = distance(obj.lr, obj.lp, r, p);
            float v = d / (obj.t - obj.lt) * 3.6;
            float c = obj.filt_coef();
            obj.v = c * obj.v + (1 - c) * v;
        }
